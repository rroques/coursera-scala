...
abstract class TweetSet {
  ...
  def ascendingByRetweet: Trending = {
    ...
    else new NonEmptyTrending(this.findMin, this.remove(this.findMin).ascendingByRetweet)
  }
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter( tw => google.exists (elt => tw.text.contains(elt)) )

  val appleTweets: TweetSet = TweetReader.allTweets.filter( tw => apple.exists (elt => tw.text.contains(elt)) )
