...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    explored: Set[Block]): Stream[(Block, List[Move])] = neighbors.filter(p => explored.find(r => r == p._1).isEmpty)
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      if (newPaths.isEmpty) initial
      else initial ++ from(newPaths, explored ++ newPaths.map(f => f._1))
    }
  }
